...
export default async function handler(
  ...
  try {
    ...
    const highConfidenceLeads = leads.filter(lead => lead.confidence_score !== null && lead.confidence_score >= 0.8).length;
    const mediumConfidenceLeads = leads.filter(lead => lead.confidence_score !== null && lead.confidence_score >= 0.5 && lead.confidence_score < 0.8).length;
    const lowConfidenceLeads = leads.filter(lead => lead.confidence_score !== null && lead.confidence_score < 0.5).length;
