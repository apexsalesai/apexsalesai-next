...
import { useEntraSession } from "@/hooks/useEntraSession";
...
export default function EchoBreakerClient() {
  const { session: entraSession, signIn, signOut } = useEntraSession();
  const session = entraSession.user ? { user: entraSession.user } : null;
  const status = entraSession.loading ? 'loading' : (entraSession.user ? 'authenticated' : 'unauthenticated');
              ...
              <div className="relative">
                ...
                >
                  <div className="p-2">
                    <button
                      onClick={() => signIn()}
                      className="w-full px-4 py-3 rounded-lg bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white text-sm transition-colors flex items-center gap-3"
                    >
